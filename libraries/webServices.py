...
class UDP_CL:
    ...
    def transmisja(self, messag, adres):
        ...
        if(messag.find('swiatlokuchni.') != -1):  # KUCHNIA
            pocz=messag.find(".")+1
            sterowanieOswietleniem(self.AdresKuchnia,messag[pocz])
        ...
        if(messag.find('reflektor1.') != -1): # REFLEKTOR LED COLOR
            ...
            sterowanieOswietleniem(self.AdresLampa1,lampa1Pok1.Jasnosc)
        ...
        if(messag.find('hydroponika.') != -1): # Hydroponika
            pocz=messag.find(".")+1
            dekoUsb.FlagaSterowanieManualne=True
            sterowanieOswietleniem(self.AdresHydroponika,messag[pocz])
        if(messag=='?m'):
            try:
        ...
        if(messag.find('sterTV.') != -1):
            ...
            sterowanieOswietleniem(self.AdresLedTV,lampaTV.Jasnosc)
            lampaTV.FlagaSterowanieManualne=True
        if(messag.find('sterTVjasnosc.') != -1):
            zmien=messag[14:17]
            if int(zmien)>0:
                lampaTV.Jasnosc=int(zmien)
            sterowanieOswietleniem(self.AdresLedTV,zmien)
            lampaTV.FlagaSterowanieManualne=True
        ...
        if(messag.find('spij') != -1):
            sterowanieOswietleniem(self.AdresLedTV,"000")
            lampaTV.FlagaSterowanieManualne=True
